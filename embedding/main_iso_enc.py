...
def get_iso_loss(f,z):
    J = jacobian_of_f(f, z)
    pullback_metric = J.permute(0, 2, 1)@J
    lambdas = torch.linalg.eigvalsh(pullback_metric)
    losses = torch.sum((lambdas - 1)**2, dim=1)
    return losses.mean()

def get_iso_loss_enc(f,z,traj_size):
    z = z.view(-1, traj_size)
    J = jacobian_of_f(f, z)
    pullback_metric = J.permute(0, 2, 1)@J
    lambdas = torch.linalg.eigvalsh(pullback_metric)
    losses = torch.sum((lambdas - 1)**2, dim=1)
...
def train(epoch):
    ...
    for batch_idx in range(epoch_size // args.batch_size):
        ...
        iso_enc_loss = get_iso_loss_enc(model.encode_sample,actions,traj_size)
        loss = loss + 1e-4*iso_enc_loss
        train_iso += iso_enc_loss
...
def build_decoder():
    ...
    for epoch in range(args.decoder_epochs):
        ...
        for batch_idx in range(dec_epoch_size):
            ...
            beta = 1e-4
            iso_loss = get_iso_loss(decoder,z)
            #print(iso_loss)
            loss = recon_loss + args.norm_loss * norm_loss + beta*iso_loss
            ...
            decoder_iso_loss += iso_loss.item()

        print((
            'ActionDecoder epoch: {}\tAverage loss: {:.4f}'
            '\tRecon loss: {:.6f}\tNorm loss: {:.6f}\tIso loss: {:.6f}'
        ).format(
          ...
          decoder_iso_loss / (dec_epoch_size * args.batch_size)))
...
def save_decoder():
    ...
    torch.save(decoder, path.join(result_path, 'decoder_iso_enc.pt'))
    return decoder

def save_model():
    torch.save(model, path.join(result_path, 'model_iso_enc.pt'))

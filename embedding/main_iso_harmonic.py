...
def get_iso_loss(f,z):
    ...
    losses = torch.sum(lambdas, dim=1)
    return losses.mean()
...
def save_decoder():
    ...
    torch.save(decoder, path.join(result_path, 'decoder_iso_harmonic.pt'))
    return decoder

def save_model():
    torch.save(model, path.join(result_path, 'model_iso_harmonic.pt'))

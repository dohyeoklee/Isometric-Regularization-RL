...
def get_iso_loss(f,z):
    ...
    losses = torch.sum((torch.log(lambdas))**2, dim=1)
    return losses.mean()
...
def save_decoder():
    ...
    torch.save(decoder, path.join(result_path, 'decoder_iso_log.pt'))
    return decoder

def save_model():
    torch.save(model, path.join(result_path, 'model_iso_log.pt'))
